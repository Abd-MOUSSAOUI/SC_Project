...
{
    key_t k;
    if((k = ftok("/etc/passwd", 0)) == -1) error("ftok");
    return k;
}
...
int create_shared_memory(void)
{
    ...
    if (shmid > -1)
    {
        ...
        fflush(stderr);
        if(shmctl(shmid, IPC_RMID, NULL) == -1) error("shmctl");
    }
    else
    {
        if(errno != ENOENT) error("shmget");
    }
...
int get_shared_memory_id(void)
{
    key_t k = key();
    int shmid;
    if((shmid = shmget(k, 0, 0)) == -1) error("shmget");
    //DEBUGF("shmid = %d", shmid);
    return shmid;
}
...
struct port *get_port(int shmid, bool readonly)
{
    ...
    void *p;
    if((p = shmat(shmid, NULL, shmflg)) == (void *)-1) error("shmat");
...
int create_semaphore(void)
{
    ...
    if (semid > -1)
    {
        ...
        fflush(stderr);
        if(semctl(semid, 0, IPC_RMID) == -1) error("semctl");
    }
    else
    {
        if(errno != ENOENT) error("semget");
    }

    if((semid = semget(k, 4, IPC_CREAT | 0666)) == -1) error("semget");
    if(semctl(semid, 0, SETVAL, 1)== -1) error("semctl"); // 1 = Closed
...
int get_semaphore(void)
{
    key_t k = key();
    int semid;
    if((semid = semget(k, 0, 0)) == -1) error("semget");
    //DEBUGF("semid = %d", semid);
    return semid;
}
...
{
    int val;
    if((val = semctl(semid, which, GETVAL)) == -1) error("semctl");
    return val;
}

void set_semaphore_value(int semid, unsigned short which, unsigned short value)
{
    if(semctl(semid, which, SETVAL, value) == -1) error("semctl");
}
